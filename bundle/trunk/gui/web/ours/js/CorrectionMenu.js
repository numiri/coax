...
CorrectionMenu.initialize = function() {
	...
	$.get(
	Editor.symbol_tree, function(data, textStatus, xmlhttp) {
		//console.log("url: " + url);
...
CorrectionMenu.populateCategoryList = function(list_div, node, start_index) {
	...
	if (CorrectionMenu.symbol_tree.current.children_type == NodeType.Category) {
		...
		if (CorrectionMenu.symbol_tree.current != CorrectionMenu.symbol_tree.root) CorrectionMenu.up.innerHTML = "Up (" + CorrectionMenu.symbol_tree.current.parent.category + ")";
		else
...
CorrectionMenu.show = function() {
	if (Editor.state != EditorState.SegmentsSelected) return;

	Editor.state = EditorState.Relabeling;

	CorrectionMenu.menu.style.display = "block";
	if (CorrectionMenu.symbol_tree.current.children_type == NodeType.Symbol) {
		CorrectionMenu.center_panel.removeChild(CorrectionMenu.current_grid);
	...
	if (all_same) {
		...
		for (var k = 0; k < rec_result.results; k++) {
			...
			if (typeof(symbol_node.symbol) == "undefined") symbol_node.symbol = symbol_node.name;
...
CorrectionMenu.select_category = function(e) {
	...
	if (CorrectionMenu.touch_moving == true) return;
	...
	if (CorrectionMenu.symbol_tree.current.children_type == NodeType.Category) CorrectionMenu.populateCategoryList(CorrectionMenu.current_list, CorrectionMenu.symbol_tree.current, 0);
...
CorrectionMenu.hide = function() {
	...
	CorrectionMenu.menu.style.display = "none";
...
CorrectionMenu.touchmove = function(e) {
	...
	if (CorrectionMenu.symbol_tree.current.children_type == NodeType.Category) {
		////console.log("chieu dai " + CorrectionMenu.current_list.childNodes.length);
		if (CorrectionMenu.current_list.childNodes.length <= 5) return;
		...
		if (CorrectionMenu.current_grid.childNodes.length <= 9) return;
		to_move = CorrectionMenu.current_grid;
		if (CorrectionMenu.current_grid.childNodes.length % 3 != 0) to_move_height = (Math.floor(CorrectionMenu.current_grid.childNodes.length / 3) + 1) * (center_height / 3.0);
		else
...
CorrectionMenu.animate = function() {
	...
	if (CorrectionMenu.div_speed > 0) {
		...
		if (CorrectionMenu.div_speed < 0) CorrectionMenu.div_speed = 0;
