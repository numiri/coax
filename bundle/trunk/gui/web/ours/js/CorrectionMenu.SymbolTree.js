...
SymbolTree.prototype.toString = function() {
	...
	while (stack.length > 0) {
		...
		sb.append(' ');
		if (node instanceof SymbolNode) sb.append(node.symbol);
...
SymbolTree.parseXml = function(in_xml) {
	//var xmldoc = new DOMParser().parseFromString(in_xml, "text/xml");
        console.log( "in_xml = " + in_xml.URL );
        var xmldoc = (in_xml.URL != null) ? in_xml 
        :   new DOMParser().parseFromString(in_xml, "text/xml");
//	var xmldoc = in_xml;
	...
	if (child_elements.length == 1) {
		if (child_elements[0].nodeName == "Category") {
			...
			while (stack.length > 0) {
				...
				if (xml_node.childNodes[k].nodeType == 1) child_elements.push(xml_node.childNodes[k]);
					...
					for (var k = 0; k < child_elements.length; k++) {
						if (child_elements[k].nodeName == "Category") {
							...
							if (child_elements[k].childNodes.length == 0) {
								...
								sym.tag = child_elements[k].getAttribute("tag") != undefined ? child_elements[k].getAttribute("tag") : "";
								sym.value = child_elements[k].getAttribute("value") != undefined ? child_elements[k].getAttribute("value") : "";
								sym.id = child_elements[k].getAttribute("id") != undefined ? child_elements[k].getAttribute("id") : "";
								sym.type = child_elements[k].getAttribute("type") != undefined ? child_elements[k].getAttribute("type") : "";
								sym.nameTag = child_elements[k].getAttribute("nameTag") != undefined ? child_elements[k].getAttribute("nameTag") : "";
								sym.event = child_elements[k].getAttribute("event") != undefined ? child_elements[k].getAttribute("event") : "";
								sym.functionName = child_elements[k].getAttribute("function") != undefined ? child_elements[k].getAttribute("function") : "";
								sym.defaultvalue = child_elements[k].getAttribute("default") != undefined ? child_elements[k].getAttribute("default") : "";
								sym.css = child_elements[k].getAttribute("css") != undefined ? child_elements[k].getAttribute("css") : "";
								
								if (sym.defaultvalue != "") {
									SettingsMenu.loadInfo(sym.id, sym.defaultvalue);
								}
								
								if (sym.tag == "" && sym.value == "" && sym.event == "") {
